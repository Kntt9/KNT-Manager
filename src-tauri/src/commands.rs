...
// Returns [{ id, startedAt }] where startedAt (ms epoch) is the real moment
// the instance began: the process creation time when it can be read (survives
// app restarts via instances.json), otherwise the known launch time
// (ready_at minus the launch grace window).
#[tauri::command]
pub async fn roblox_watched_ids(state: State<'_, AppState>) -> Result<Vec<serde_json::Value>, ()> {
    ...
    for (id, ready_at) in watched.iter() {
        let mut started_at = ready_at - crate::native::LAUNCH_DELAY_MS;
        if let Some(pid) = pids.get(id) {
            if let Some(ft) = crate::native::process_start_time(*pid) {
                // FILETIME (100ns since 1601-01-01) -> ms since Unix epoch.
                let epoch_ms = (ft / 10_000) as i64 - 116_444_736_00000i64;
                if epoch_ms > 0 {
                    started_at = epoch_ms;
                }
            }
        }
        out.push(serde_json::json!({ "id": id, "startedAt": started_at }));
    }
    Ok(out)
}
...
// Accounts whose game went to the Roblox "home" (tray) — game process died
// but a hidden launcher window is still around. The UI shows these amber.
#[tauri::command]
pub fn roblox_home_ids(state: State<'_, AppState>) -> Vec<String> {
    state
        .home_accounts
        .lock()
        .unwrap()
...
// Closes the shared Roblox "home" (tray) launcher and clears every amber badge.
#[tauri::command]
