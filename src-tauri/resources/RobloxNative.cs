...
internal static class RobloxNative
{
    ...
    // Lists RobloxPlayerBeta PIDs that are sitting in the "home" (tray):
    // the process owns at least one top-level window but NONE of them is
    // visible. A live game window is visible; after the user closes the
    // game with the X, the launcher home window is hidden, so this is how
    // the app tells "the instance went to home" apart from "the instance
    // really closed".
    ...
    internal static string HomeList()
    {
        ...
        foreach (var p in Process.GetProcessesByName("RobloxPlayerBeta"))
        {
            try
            {
                ...
                EnumWindows((hWnd, lParam) =>
                {
                    ...
                    {
                        ownsWindow = true;
                        if (IsWindowVisible(hWnd)) { anyVisible = true; return false; }
                    }
                    return true;
                }, IntPtr.Zero);
                // Has at least one window (so it is the launcher/home, not a
                // headless stub) and none visible -> "home".
                if (ownsWindow && !anyVisible) hidden.Add(p.Id.ToString());
            }
            catch { }
        }
    ...
    private static int RunHome()
    {
        Console.Out.WriteLine(HomeList());
        Console.Out.Flush();
        return 0;
    }
    ...
    static void Handle(string line)
    {
        ...
        try
        {
            switch (cmd)
            {
                ...
                case "home":
                    Reply(id, RobloxNative.HomeList());
                    break;
